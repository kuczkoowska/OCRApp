...
class OCRTrainer:
    ...
    def train(self, train_dataset, val_dataset, epochs=50, save_path='models/'):
        """
...
def main():
    ...
    config = {
        'mjsynth_path': '/home/jagoda/studia/inteloblicz/OCRApp/mjsynth',  # Ścieżka do zbioru danych MJSynth
        'batch_size': 32,  # Rozmiar batcha dla treningu
        'epochs': 50,  # Liczba epok
        'img_height': 32,  # Wysokość obrazu do zmiany rozmiaru
        'img_width': 128,  # Szerokość obrazu do zmiany rozmiaru
        'max_samples': None  # Ograniczenie rozmiaru zbioru danych do testów (None oznacza brak ograniczenia)
    }
    ...
    train_samples = data_loader.load_data('train')

    print("Ładowanie danych walidacyjnych...")
    val_samples = data_loader.load_data('val')
    ...
    if config['max_samples']:
        ...
        val_samples = val_samples[:config['max_samples']//5]
        test_samples = test_samples[:config['max_samples']//10]
